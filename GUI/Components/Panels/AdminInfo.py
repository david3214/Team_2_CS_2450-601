...
class AdminInfo(Info):
    def __init__(self, master: 'Profile', bgColor: str = btn_color, editable: bool = False) -> None:
        ...
        self.values = [master.emp.PayMethod, master.emp.BankInfo, master.emp.Route, master.emp.Salary, master.emp.Hourly, master.emp.Commission, master.emp.DOB.date(), master.emp.SSNum] if master.emp else ['' for _ in range(len(self.fields))]
        ...
        if self.editable:
            ...
            for i, wrapper in enumerate(self.validationWrappers):
                self.entries[self.validationMethods[i][1]].configure(**dict(validatecommand=wrapper, validate='all'))

            self.entries[6].configure(**dict(validatecommand=self.dateWrapper, validate='focusout'))
